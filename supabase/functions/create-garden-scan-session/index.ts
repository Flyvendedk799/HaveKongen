...
Deno.serve(async (req) => {
  ...
  const guarded = await guard(req, { fn: "create-garden-scan-session", limit: 30, windowSeconds: 60, skipIdentity: true });
  ...
  try {
    ...
    const captureMetadata = {
      ...
      minimum_route_steps: 4,
      recommended_route_steps: 4,
      manual_anchor_mode: "optional",
      minimum_anchor_count: 0,
      recommended_anchor_count: 2,
    ...
    return json({
      ...
      manifest_schema: {
        version: 1,
        required: ["session_id", "garden_id", "device", "capture", "route", "tracking", "keyframes"],
      ...
      pipeline_contract: {
        ...
        quality_gates: {
          minimum_anchors: 0,
          anchors_are_manual: true,
          anchors_required_for_upload: false,
          minimum_anchors_for_high_confidence: 2,
          recommended_anchors: 2,
          minimum_anchor_spread_m: 3,
          recommended_anchor_spread_m: 8,
          minimum_route_steps: 4,
          route_camera_target: "garden_center",
          minimum_keyframes: 8,
